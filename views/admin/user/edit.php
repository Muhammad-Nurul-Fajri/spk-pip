...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nama) || empty($username) || empty($role)) {
        $pesan_error = 'Nama, Username, dan Role wajib diisi!';
    } else {
...
.form-control, .form-select { height: 42px; border-radius: 10px; border: 1px solid #d6e4d3; background: #fafdf8; padding-left: 14px; font-size: 13px; }
.form-control:focus, .form-select:focus { border-color: #C6D166; box-shadow: 0 0 0 0.15rem rgba(76,175,80,0.15); background: white; }
        ...
        <form action="edit.php?id=<?php echo $id; ?>" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="role" class="form-select" required>
                    ...
                    <option value="ketua_yayasan" <?php echo ($data['role'] == 'ketua_yayasan') ? 'selected' : ''; ?>>Ketua Yayasan</option>
                    <option value="siswa" <?php echo ($data['role'] == 'siswa') ? 'selected' : ''; ?>>Siswa</option>
                </select>
            </div>
