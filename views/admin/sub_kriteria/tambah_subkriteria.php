...
$pesan_error  = '';
$selected_kriteria = isset($_GET['id_kriteria']) ? intval($_GET['id_kriteria']) : 0;

// Ambil data kriteria untuk dropdown
$kriteria = mysqli_query($koneksi, "SELECT * FROM kriteria ORDER BY kode_kriteria ASC");
if (!$kriteria) {
    die("Error ambil kriteria: " . mysqli_error($koneksi));
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    
    $id_kriteria = intval($_POST['id_kriteria'] ?? $selected_kriteria);
    ...
    if ($id_kriteria <= 0) {
        ...
        $pesan_error = 'Nilai wajib diisi!';
    } elseif (!is_numeric($nilai) || $nilai < 0) {
        $pesan_error = 'Nilai harus angka positif!';
    } else {
        
        $cek = mysqli_query($koneksi, "SELECT id FROM sub_kriteria WHERE id_kriteria = $id_kriteria AND nama_sub = '$nama_sub'");
        if (!$cek) {
...
<div class="sidebar">
    ...
    <div class="logo">
        ...
        <img src="../../../public/assets/img/logo.png" onerror="this.style.display='none'">
    ...
    <ul class="menu">
        ...
        <li><a href="index.php" class="active"><i class="fa fa-layer-group"></i> Data Sub Kriteria</a></li>
...
<div class="content">
    ...
    <div class="card-custom">
        ...
        <div class="card-header-custom">
            <h5>Form Tambah Data Sub Kriteria</h5>
        </div>
        ...
        <form method="POST" action="">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <?php if ($selected_kriteria > 0): 
                        // Ambil nama kriteria yang dipilih
                        $kriteria_info = mysqli_query($koneksi, "SELECT * FROM kriteria WHERE id = $selected_kriteria");
                        $k_info = mysqli_fetch_assoc($kriteria_info);
                    ?>
                    <input type="hidden" name="id_kriteria" value="<?php echo $selected_kriteria; ?>">
                    <input type="text" class="form-control" value="<?php echo htmlspecialchars($k_info['kode_kriteria'] . ' - ' . $k_info['nama_kriteria'] . ' (' . $k_info['jenis'] . ')'); ?>" readonly>
                    <?php else: ?>
                    <select name="id_kriteria" class="form-select" required>
                        ...
                        ?>
                        <option value="" disabled>Belum ada kriteria</option>
                        <?php endif; ?>
                    </select>
                    <?php if (mysqli_num_rows($kriteria) == 0): ?>
                    <small class="text-danger" style="font-size: 12px;">Silakan tambah kriteria terlebih dahulu</small>
                    <?php endif; ?>
                    <?php endif; ?>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="nama_sub" class="form-control" placeholder="Contoh: Sangat Baik, Baik, Cukup" value="<?php echo htmlspecialchars($nama_sub ?? ''); ?>" required>
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label">Nilai <span class="required">*</span></label>
                    <input type="number" name="nilai" class="form-control" placeholder="Masukkan nilai" step="1" min="0" value="<?php echo htmlspecialchars($nilai ?? ''); ?>" required>
            ...
            <div class="d-flex gap-2 mt-3">
                <button type="submit" class="btn-simpan" <?php echo mysqli_num_rows($kriteria) == 0 ? 'disabled' : ''; ?>><i class="fa fa-save me-2"></i>Simpan</button>
