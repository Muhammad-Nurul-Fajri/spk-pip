...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($kode_alternatif)) {
        ...
        // Cek duplikat kode
        $cek = mysqli_query($koneksi, "SELECT id FROM siswa WHERE kode_alternatif = '$kode_alternatif'");
        if (mysqli_num_rows($cek) > 0) {
            $pesan_error = 'Kode alternatif sudah ada!';
        } else {
            $simpan = mysqli_query($koneksi,
                "INSERT INTO siswa (kode_alternatif, nama, kelas) VALUES ('$kode_alternatif', '$nama', '$kelas')"
            );
            if ($simpan) {
                // Insert default penilaian for all criteria
                ...
                while ($kr = mysqli_fetch_assoc($kriterias)) {
                    mysqli_query($koneksi, "INSERT INTO penilaian (id_siswa, id_kriteria, nilai) VALUES ($id_siswa, {$kr['id']}, 1)");
                }
        ...
        <form action="tambah.php" method="POST">
            <div class="mb-3">
                ...
                <input type="text" name="kode_alternatif" class="form-control" placeholder="Contoh: A1" required
                       value="<?php echo htmlspecialchars($_POST['kode_alternatif'] ?? ''); ?>">
            </div>
            <div class="mb-3">
